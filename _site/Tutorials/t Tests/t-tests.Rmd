...
**Hint:** For a quick review, refer to "Using t.test()" subsection in the tutorial.
</div>

```{r exercise1-check}
grade_result(
  pass_if(~ identical(.result, t.test(teengamb$income, mu = 5, alternative = 'less')), "Good job!"),
  fail_if(~ identical(.result, t.test(teengamb$income, mu = 5)), "Oops looks like you missed to specify the alternative hypothesis!"),
  fail_if(~ identical(.result, t.test(teengamb$income, alternative = 'less')), "You are expected to test the hypothesis against mu = 5"),
  fail_if(~ TRUE, "Not quite, refer to the section on 'Using t.test()' ")
)
```
...
**Hint:** In case you need a little help, go back to "Independent Samples t-test". 
</div>

```{r exercise3-check}
grade_result(
  pass_if(~ identical(.result, t.test(status ~ sex, data = teengamb)), "Good job!"),
  fail_if(~ identical(.result, t.test(sex ~ status, data = teengamb)), "Looks like you misplaced the variables, refer to 'Independent Sample t-test' for a quick review."),
  pass_if(~ identical(.result, t.test(status ~ sex, data = teengamb, alternative = "two.sided")), "Good job! But you don't need to specify the alternative because it's two-sided by default!"),
  fail_if(~ TRUE, "Not quite. Refer back to 'Independent Sample t-test'. ")
)
```

...
```{r, echo=TRUE}
mean(iris$Sepal.Length) # computes the arithmetic mean of the sepal length column from the iris data
sd(iris$Sepal.Length)  #computes the standard deviation of the sepal length column from the iris data
```
...
grade_result(
  ...
  fail_if(~ identical(.result, mean(iris$Sepal.Length)), "You are expected to calculate the median,check the function you are using!"),
  fail_if(~ identical(.result, median(iris)), "Oops! Looks like you are not passing vector into the function!"),
...
<div id = "practice2-hint">
**Hint:** Remember how we add titles to plots? If not, visit the subsection "Visualizing Numerical Variables" for a quick review.
</div>

```{r practice2-solution, practice2.reveal_solution = FALSE}
...
grade_result(
  ...
  fail_if(~ identical(.result, mean(mtcars$cyl[mtcars$vs == 1])), "Oops looks like these cars do not have a V shaped engine!" ),
  fail_if(~ TRUE, "Revisit the data frame tutorial to check how the subsetting should be done!")
)
```
...
grade_result(
  ...
  fail_if(~ TRUE, "Use hist(properly sliced data, breaks = 15)")
)
```
